...
type IntroStage = "first" | "second" | "event";
...
export default function Home() {
  ...
  function enterEventPage() {
    setIntroStage("event");
    window.setTimeout(() => {
      document.getElementById("event-page")?.scrollIntoView({ behavior: "smooth", block: "start" });
    }, 0);
  }
  ...
  return (
    <main className="site-shell">
      {introStage !== "event" && (
        <section className="intro-sequence" aria-label="After Hours invitation sequence">
          <IntroVideo
            id="intro-first"
            src="/first.mp4"
            title="After Hours invitation"
            nextId="intro-second"
            onAction={enterSecond}
          />
          {introStage === "second" ? (
            <IntroVideo
              id="intro-second"
              src="/second.mp4"
              title="After Hours event transition"
              onEnded={enterEventPage}
            />
          ) : null}
        </section>
      )}
