...
function AccessPinSettings() {
  const [adminPin, setAdminPin] = useState(""); const [designerPin, setDesignerPin] = useState(""); const [eventDayPin, setEventDayPin] = useState(""); const [status, setStatus] = useState("Loading PINs…"); const [saving, setSaving] = useState(false);
  useEffect(() => { void fetch("/api/admin/access-pins", { cache: "no-store" }).then(async (response) => { const pins = await response.json() as { adminPin?: string; designerPin?: string; eventDayPin?: string }; if (!response.ok) throw new Error(); setAdminPin(pins.adminPin || ""); setDesignerPin(pins.designerPin || ""); setEventDayPin(pins.eventDayPin || ""); setStatus(""); }).catch(() => setStatus("Could not load PINs.")); }, []);
  async function save(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setSaving(true); setStatus(""); try { const response = await fetch("/api/admin/access-pins", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ adminPin, designerPin, eventDayPin }) }); const result = await response.json() as { error?: string }; if (!response.ok) throw new Error(result.error || "Could not save PINs."); setStatus("PINs saved."); } catch (saveError) { setStatus(saveError instanceof Error ? saveError.message : "Could not save PINs."); } finally { setSaving(false); } }
  return <form className={styles.pinSettings} onSubmit={save}><div><strong>Section PINs</strong><span>Set separate access for Admin login, Designer, and Event Day.</span></div><label><span>Admin login PIN</span><input type="text" inputMode="numeric" pattern="[0-9]{4}" maxLength={4} value={adminPin} onChange={(event) => setAdminPin(event.target.value.replace(/\D/g, "").slice(0, 4))} required /></label><label><span>Designer PIN</span><input type="text" inputMode="numeric" pattern="[0-9]{4}" maxLength={4} value={designerPin} onChange={(event) => setDesignerPin(event.target.value.replace(/\D/g, "").slice(0, 4))} required /></label><label><span>Event Day PIN</span><input type="text" inputMode="numeric" pattern="[0-9]{4}" maxLength={4} value={eventDayPin} onChange={(event) => setEventDayPin(event.target.value.replace(/\D/g, "").slice(0, 4))} required /></label><button className={styles.primaryButton} disabled={saving}>{saving ? "Saving…" : "Save PINs"}</button>{status ? <small role="status">{status}</small> : null}</form>;
}
