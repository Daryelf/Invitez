...
export const DEFAULT_RSVP_LAYOUT: RsvpLayout = {
  ...
  countdown: { top: 91.25, left: 11.36, width: 80, height: 8, rotation: 0 },
};
...
export function normalizeRsvpLayout(value: unknown): RsvpLayout {
  ...
  return Object.fromEntries(RSVP_LAYOUT_KEYS.map((key) => {
    ...
    const usesPreviousCountdownDefault = key === "countdown"
      && (finiteNumber(candidate.top, Number.NaN) === 89.5 || finiteNumber(candidate.top, Number.NaN) === 90.25)
      && (finiteNumber(candidate.left, Number.NaN) === 10 || finiteNumber(candidate.left, Number.NaN) === 11.36)
